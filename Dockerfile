...
# Tested on 5 January 2018 with Docker Version 17.09.1-ce-mac42 (21090)
FROM ubuntu:16.04
MAINTAINER <david.pinney@nreca.coop>

# Install and setup OMF reqs
RUN apt-get -y update && apt-get install -y python-pip git unixodbc-dev libfreetype6-dev pkg-config python-dev python-numpy alien python-pygraphviz python-pydot ffmpeg mdbtools python-cairocffi octave graphviz
