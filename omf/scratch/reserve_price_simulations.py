...
import pandas as pd
# Run simulations
#format min, mean, max, stdev, %chance price point is 0 all in $/MWh
...
class distribution:
	...
	def randomNormal(self):
		...
		s = get_truncated_normal(mean, stdev, min_val, max_val)
		s = s.rvs(8760)
...
test1 = distribution(PJM_primary)
s = test1.randomNormal()
...
for i in range(len(regulation_dist)):
	dist = distribution(regulation_dist[i])
	s = dist.randomNormal()
...
for i in range(len(spinning_dist)):
	dist = distribution(spinning_dist[i])
	s = dist.randomNormal()
...
for i in range(len(nospin_dist)):
	dist = distribution(nospin_dist[i])
	s = dist.randomNormal()
