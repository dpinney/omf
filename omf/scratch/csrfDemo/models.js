...
function cancelModel() {
	params = {user:allInputData.user,
		...
		csrfToken:gebi('csrfToken2').value}
	post_to_url("/cancelModel/", params, "POST")
}

function deleteModel() {
	if (confirm("Deleting this model cannot be undone. Continue?")){
		post_to_url("/delete/Model/"+allInputData.user+"/"+allInputData.modelName, {"csrfToken":gebi('csrfToken2').value}, "POST")
	} else {
		return false
	}
}
	...
	if (newName) {
		$.ajax({url:"/uniqObjName/Model/public/" + newName}).done(function(data) {
			if (data.exists) {
				...
				post_to_url("/publishModel/" + allInputData.user + "/" + allInputData.modelName+"/", {"newName":newName, "csrfToken":gebi('csrfToken2').value})
			}
		})
	}
}
	...
	if (newName) {
		$.ajax({url:"/uniqObjName/Model/" + allInputData.user + "/" + newName}).done(function(data) {
			if (data.exists) {
				...
				post_to_url("/duplicateModel/" + allInputData.user + "/" + allInputData.modelName+"/", {"newName":newName, "csrfToken":gebi('csrfToken2').value})
			}
		})
	}
}
