...
def work(modelDir, ind):
	...
	zipped = zip(dailyLoadPredictions, dailyPl, dailyPu, dailyEu)
	for i, (load, pl, pu, eu) in enumerate(zipped):
		peak = max(load)
		if fc.shouldDispatchDeferral(peak, df, float(ind['confidence'])/100, threshold):
			dispatched_d[i] = True
	...
	o['dataCheck'] = 'Threshold exceeded' if any([threshold > i for i in demandAdj]) else ''
	o['transformerThreshold'] = threshold
