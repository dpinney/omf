...
def _tests(companyname):
	...
	if ("chesapeake" in companyname):
		...
		modelLoc = pJoin(workDir,"admin","Chesapeake solarRates Testing")
	...
	if ("autocli" in companyname):
		...
		for key in monthlyData:
			inData[key] = monthlyData[key]
		modelLoc = pJoin(workDir,"admin","Autocli solarRates Testing")
		# Blow away old test results if necessary.
		try:
			shutil.rmtree(modelLoc)
		except:
	...
	if ("orville" in companyname):
		...
		for key in monthlyData:
			inData[key] = monthlyData[key]
		modelLoc = pJoin(workDir,"admin","Orville solarRates Testing")
		# Blow away old test results if necessary.
		try:
			shutil.rmtree(modelLoc)
		except:
	...
	if ("olin" in companyname):
		...
		for key in monthlyData:
			inData[key] = monthlyData[key]
		modelLoc = pJoin(workDir,"admin","Olin solarRates Testing")
		# Blow away old test results if necessary.
		try:
			shutil.rmtree(modelLoc)
		except:
