		...
		for line in content:
			...
			if IsTrpMeter == 1:
				if 'name' in line.strip():
					name_temp = line.strip().strip("name ").strip(";")
				if 'parent' in line.strip():
					parent_temp = line.strip().strip("parent ").strip(";")
...
def ChangeGlmFileWye(inputFileName,outputFileName,PV,PV_index):
	with open(inputFileName, 'r') as inputFile, open(outputFileName, 'w+') as outputFile:
		...
		for line in contentIn:
			...
			if isPV == 1:
				if 'name' in line.strip():
					PV_name = line.strip().strip('name ').strip(';')
					m = PV_index[PV_name]
				if 'Q_Out' in line.strip():
					line = '\t' + line.strip(';').split()[0] + ' ' +  str(PV[m].Q_Out) + ';\n'
...
def ChangeGlmFileDelta(inputFileName,outputFileName,PV,PV_index):
	with open(inputFileName, 'r') as inputFile, open(outputFileName, 'w+') as outputFile:
		...
		for line in contentIn:
			...
			if isLoad == 1:
				if 'name' in line.strip():
					load_name = line.strip().strip('name ').strip(';')
	...
	with open(inputFileName, 'r') as inputFile, open(outputFileName, 'w+') as outputFile:
		...
		for line in contentIn:
			...
			if 'stoptime' in line.strip():
					line =" 	stoptime '2000-01-01 0:00:01';\n"
			if 'object load' in line.strip():
				isLoad =1
			else:
				# not inverter, just copy
