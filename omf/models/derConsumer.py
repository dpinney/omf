...
def construct_energy_rate_array(response_file, timestamps):
	"""
	Constructs an array of hourly energy rates for an entire year (length 8760) based on the provided weekday or weekend rate schedule information.
	Expected inputs are 
...
def work(modelDir, inputDict):
	...
	if inputDict.get('urdbLabelBool') is not None:
		...
		if response.status_code == 200:
			try:
				...
				data = response.json()
				response_file = data['items'][0] ## This is a dictionary containing the utility rate structure information
				print(response_file)
	...
	else: ## If the Residential Response File is chosen instead of the URDB Label method, use the user-provided .json response file
		try:
