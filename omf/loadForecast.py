...
class svmNextDayPeakTime:
	...
	def cv_metrics(self, csv, metric, random_state = None, splits = None, *args, **kwargs):
		...
		results = self._cv_predict(csv, random_state = random_state, splits = splits if splits else 3)
	...
	def _cv_predict(self, csv = None, df = None, random_state = None, splits = 3): #cross-validated predict
		...
		ks = df.loc[:, ['tmr_temp', 'tmr_weekend', 'tmr_morning_peak', 'tmr_afternoon_peak']]
		y = df.tmr_afternoon_peak_bool.astype('bool')
		...
		for train_index, test_index in self.skf.split(ks, y):
			ks_train, ks_test, y_train, y_test = ks.iloc[train_index,:], ks.iloc[test_index,:], y.iloc[train_index], y.iloc[test_index]
			x_train, x_test = ks_train.loc[:, ['tmr_temp', 'tmr_weekend']], ks_test.loc[:, ['tmr_temp', 'tmr_weekend']]
			...
			classifier_predictions = self.classifier.predict(x_test) #returns True if afternoon
			...
			for hpm_index, index_val in enumerate(df.index[test_index][~classifier_predictions]):
