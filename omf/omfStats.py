...
def genModelDatabase(outPath):
    ...
    with open(outPath, 'wb') as statsFile:
        ...
        for owner in os.listdir(modelDir):
            ownerDir = os.path.join(modelDir, owner)
            for project in os.listdir(ownerDir):
...
def modelDatabaseStats(dataFilePath, outFilePath):
    ...
    REPIC(models[
          1:3])  #OUTPUT: [{'Status': '2017-02-15 11:11:41', 'Created': None, 'Runtime (H:M:S)': '0:03:39', 'Model Name': 'Demo gridlabMulti scadaCalibration', 'Owner': 'public', 'Type': 'gridlabMulti'}, {'Status': '2014-07-30 11:23:46', 'Created': None, 'Runtime (H:M:S)': '0:00:37', 'Model Name': 'Demo Wind Olin Barre GH Wind', 'Owner': 'public', 'Type': 'gridlabMulti'}]
    # Model count:
    ...
    REPIC(
        modelCounts)  #OUTPUT: {'solarEngineering': 1, 'solarFinancial': 1, 'cvrStatic': 1, 'solarCashflow': 1, 'gridlabMulti': 7, 'solarSunda': 1, 'voltageDrop': 3, 'storagePeakShave': 1, 'pvWatts': 1}
