...
def _readCymeSource(feederId, _type, modelDir):
	...
	if _type == 1:
		CYMSOURCE = {
			"name": None,  # information structure for each object found in CYMSOURCE
			"bustype": "SWING",
		...
		CYMEQUIVALENTSOURCE = {
			"name": None,  # information structure for each object found in CYMEQUIVALENTSOURCE
			"bustype": "SWING",
			"nominal_voltage": None,
			"phases": None,
		}
		...
		if _type == 1:
			...
			for row in feeder_db:
				if row.NodeId in feederId_equivalent:  # jfk.  logic was backwards.
					...
					cymsource[_fixName(row.NodeId)]["source_voltage"] = str(
...
def _readCymeNode(feederId, modelDir):
	...
	y_pixel_range = 800
	cymnode = {}
	CYMNODE = {"name": None, "latitude": None, "longitude": None}
	# node_db = networkDatabase.execute("SELECT NodeId, X, Y FROM CYMNODE WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
	node_db = _csvToDictList(pJoin(modelDir, "cymeCsvDump", "CYMNODE.csv"), feederId)
	if len(node_db) == 0:
		warnings.warn(
			"No information node locations were found in CYMNODE for feeder id: {:s}.".format(
				feederId
			),
			RuntimeWarning,
		)
		...
		for row in overheadbyphase_db:
			...
			if row.DeviceNumber not in cymoverheadbyphase.keys():
				...
				if row.PhaseConductorIdA != "NONE":
					overheadLineConfiguration["conductor_A"] = _fixName(
						row.PhaseConductorIdA
					)
				...
				if row.PhaseConductorIdB != "NONE":
					overheadLineConfiguration["conductor_B"] = _fixName(
						row.PhaseConductorIdB
					)
				...
				if row.PhaseConductorIdC != "NONE":
					overheadLineConfiguration["conductor_C"] = _fixName(
						row.PhaseConductorIdC
					)
				...
				if row.NeutralConductorId != "NONE":
					...
					if (
						row.NeutralConductorId != "NONE"
						and _fixName(row.NeutralConductorId) not in overheadConductors
					):
				...
				cymoverheadbyphase[row.DeviceNumber]["length"] = (
					float(row.Length) * m2ft
				)
...
def _readCymeUndergroundLine(feederId, modelDir):
	...
	CYMUNDERGROUNDLINE = {
		"name": None,  # Information structure for each object found in CYMUNDERGROUNDLINE
		"length": None,
		"cable_id": None,
	}
	...
	ug_line_db = _csvToDictList(
		pJoin(modelDir, "cymeCsvDump", "CYMUNDERGROUNDLINE.csv"), feederId
	)
	if len(ug_line_db) == 0:
		warnings.warn(
			"No underground_line objects were found in CYMUNDERGROUNDLINE for feeder_id: {:s}.".format(
				feederId
			),
			RuntimeWarning,
		)
		...
		for row in ug_line_db:
			...
			if row.DeviceNumber not in cymundergroundline.keys():
				...
				cymundergroundline[row.DeviceNumber]["name"] = _fixName(
					row.DeviceNumber
				)
				cymundergroundline[row.DeviceNumber]["cable_id"] = _fixName(row.CableId)
				cymundergroundline[row.DeviceNumber]["length"] = (
					float(row.Length) * m2ft
				)
...
def _readCymeOverheadLineUnbalanced(feederId, modelDir):
	...
	ug_line_db = _csvToDictList(
		pJoin(modelDir, "cymeCsvDump", "CYMOVERHEADLINEUNBALANCED.csv"), feederId
	)
	if len(ug_line_db) == 0:
		warnings.warn(
			"No underground_line objects were found in CYMOVERHEADLINEUNBALANCED for feeder_id: {:s}.".format(
				feederId
			),
			RuntimeWarning,
		)
		...
		for row in ug_line_db:
			...
			if row.DeviceNumber not in cymoverheadlineunbalanced.keys():
				...
				cymoverheadlineunbalanced[row.DeviceNumber]["name"] = _fixName(
					row.DeviceNumber
				)
				cymoverheadlineunbalanced[row.DeviceNumber]["configuration"] = _fixName(
					row.LineId
				)
				cymoverheadlineunbalanced[row.DeviceNumber]["length"] = (
					float(row.Length) * m2ft
				)
...
def _readCymeOverheadLine(feederId, modelDir):
	cymoverheadline = {}
	CYMOVERHEADLINE = {"name": None, "length": None, "configuration": None}
	lineIds = []
	overhead_line_db = _csvToDictList(
		pJoin(modelDir, "cymeCsvDump", "CYMOVERHEADLINE.csv"), feederId
	)
	if len(overhead_line_db) == 0:
		warnings.warn(
			"No overhead_line objects were found in CYMOVERHEADLINE for feeder_id: {:s}.".format(
				feederId
			),
			RuntimeWarning,
		)
		...
		for row in overhead_line_db:
			...
			if row.DeviceNumber not in cymoverheadline.keys():
				...
				cymoverheadline[row.DeviceNumber]["configuration"] = _fixName(
					row.LineId
				)
...
def _readCymeQOverheadLine(feederId, modelDir):
	...
	spacingIds = []
	cymeqoverheadline_db = _csvToDictList(
	...
	if len(cymeqoverheadline_db) == 0:
		...
		for row in cymeqoverheadline_db:
			...
			if row.EquipmentId not in cymeqoverheadline.keys():
				...
				cymeqoverheadline[row.EquipmentId]["configuration"] = _fixName(
					row.PhaseConductorId
				)
				cymeqoverheadline[row.EquipmentId]["spacing"] = _fixName(
					row.ConductorSpacingId
				)
				cymeqoverheadline[row.EquipmentId]["conductor_N"] = _fixName(
					row.NeutralConductorId
				)
...
def _readCymeReactors(feederId, modelDir):
	cymseriesreactor = {}
	CYMSERIESREACTOR = {"name": None, "configuration": None}
	reactorIds = []
	...
	if len(seriesreactor_db) == 0:
		...
		for row in seriesreactor_db:
			...
			if row.DeviceNumber not in cymseriesreactor.keys():
				...
				cymseriesreactor[row.DeviceNumber]["configuration"] = _fixName(
					row.EquipmentId
				)
...
def _readEqReactors(feederId, modelDir):
	cymeqreactor = {}
	CYMEQREACTOR = {"name": None, "reactance": None}
	cymeqreactor_db = _csvToDictList(
...
def _splitLinkObjects(sectionDict, deviceDict, linkDict, overheadDict, undergroundDict):
	...
	for link in linkDict.keys():
		if (
			link in overheadDict.keys() or link in undergroundDict.keys()
			...
			for newLinkId in newLinkIds:  # jfk
				if (
					deviceDict[newLinkId]["location"] == 1
...
def _readCymeRecloser(feederId, modelDir):
	cymrecloser = {}
	CYMRECLOSER = {"name": None, "status": None}
	...
	if len(regulator_db) == 0:
		...
		for row in regulator_db:
			...
			if row.DeviceNumber not in cymregulator.keys():
				...
				cymregulator[row.DeviceNumber]["regulation"] = (
					float(row.BoostPercent) / 100.0
				)
			...
			if row.SwitchedKVARA is None or float(row.SwitchedKVARA) == 0.0:
				...
				if row.DeviceNumber not in cymshuntcapacitor.keys():
					...
					cymshuntcapacitor[row.DeviceNumber][
						...
						"ABC"
					)  # _convertPhase(int(row.Phase)) #jfk. Painful change.  Phase doesn't exist in my capacitor tables.
					cymshuntcapacitor[row.DeviceNumber]["status"] = row.Status
					if (
						float(row.KVARA) == 0.0
						and float(row.KVARA) == 0.0
						and float(row.KVARA) == 0.0
						and float(row.KVARABC) > 0.0
					):
						cymshuntcapacitor[row.DeviceNumber]["capacitor_A"] = (
							float(row.KVARABC) * 1000 / 3
						)
						cymshuntcapacitor[row.DeviceNumber]["capacitor_B"] = (
							float(row.KVARABC) * 1000 / 3
						)
						cymshuntcapacitor[row.DeviceNumber]["capacitor_C"] = (
							float(row.KVARABC) * 1000 / 3
						)
					else:
						if float(row.KVARA) > 0.0:
							cymshuntcapacitor[row.DeviceNumber]["capacitor_A"] = (
								float(row.KVARA) * 1000
							)
						if float(row.KVARB) > 0.0:
							cymshuntcapacitor[row.DeviceNumber]["capacitor_B"] = (
								float(row.KVARB) * 1000
							)
						if float(row.KVARC) > 0.0:
							cymshuntcapacitor[row.DeviceNumber]["capacitor_C"] = (
								float(row.KVARC) * 1000
							)
					if float(row.KVLN) > 0.0:
						cymshuntcapacitor[row.DeviceNumber]["kV_line_neutral"] = (
							float(row.KVLN) * 1000
						)
					if int(row.CapacitorControlType) == 2:
						cymshuntcapacitor[row.DeviceNumber]["control"] = "VAR"
						cymshuntcapacitor[row.DeviceNumber]["VAr_set_high"] = (
							float(row.OnValue) * 1000
						)
						cymshuntcapacitor[row.DeviceNumber]["VAr_set_low"] = (
							float(row.OffValue) * 1000
						)
						cymshuntcapacitor[row.DeviceNumber]["pt_phase"] = _convertPhase(
							int(row.Phase)
						)
						...
						cymshuntcapacitor[row.DeviceNumber]["voltage_set_low"] = float(
							row["OnValue" + controlledphase]
						)
						cymshuntcapacitor[row.DeviceNumber]["voltage_set_high"] = float(
							row["OffValue" + controlledphase]
						)
						cymshuntcapacitor[row.DeviceNumber][
							"pt_phase"
						] = controlledphase
						cymshuntcapacitor[row.DeviceNumber]["remote_sense"] = _fixName(
						...
						cymshuntcapacitor[row.DeviceNumber][
							"pt_phase"
						] = "ABC"  # _convertPhase(int(row.Phase)) #doesn't exist
						cymshuntcapacitor[row.DeviceNumber]["voltage_set_high"] = (
							float(row.KVLN) * 1000
						)
						cymshuntcapacitor[row.DeviceNumber]["voltage_set_low"] = (
							float(row.KVLN) * 1000
						)
			else:
				row.DeviceNumber = _fixName(row.DeviceNumber)
				...
				if row.DeviceNumber not in cymshuntcapacitor.keys():
					...
					if float(row.SwitchedKVARA) > 0.0:
						cymshuntcapacitor[row.DeviceNumber]["capacitor_A"] = (
							float(row.SwitchedKVARA) * 1000
						)
					if float(row.SwitchedKVARB) > 0.0:
						cymshuntcapacitor[row.DeviceNumber]["capacitor_B"] = (
							float(row.SwitchedKVARB) * 1000
						)
					if float(row.SwitchedKVARC) > 0.0:
						cymshuntcapacitor[row.DeviceNumber]["capacitor_C"] = (
							float(row.SwitchedKVARC) * 1000
						)
					if float(row.KVLN) > 0.0:
						cymshuntcapacitor[row.DeviceNumber]["kV_line_neutral"] = (
							float(row.KVLN) * 1000
					...
					if int(row.CapacitorControlType) == 2:
						...
						cymshuntcapacitor[row.DeviceNumber][
							"pt_phase"
						] = controlledphase  # row.Phase doesn't exist
						if row.ControlledNodeId:
							cymshuntcapacitor[row.DeviceNumber][
								"remote_sense"
							] = _fixName(row.ControlledNodeId)
	...
	if len(customerload_db) == 0:
		...
		for row in customerload_db:
			...
			if row.DeviceNumber not in cymcustomerload.keys():
				...
				cymcustomerload[row.DeviceNumber]["phases"] = _cleanPhases(
				...
				if int(row.Phase) == 1:
					...
					cymcustomerload[row.DeviceNumber][
						"constant_power_A"
					] = _setConstantPower(float(row.LoadValue2), load_real, load_imag)
				...
				[load_real, load_imag] = _determineLoad(
					...
					float(row.LoadValue1),
					float(row.LoadValue2),
					float(row.ConnectedKVA),
				)
				ph = cymcustomerload[row.DeviceNumber]["phases"] + _convertPhase(
					int(row.Phase)
				)
				...
				if int(row.Phase) == 1:
					...
					cymcustomerload[row.DeviceNumber][
						...
						float(row.LoadValue2),
						cymcustomerload[row.DeviceNumber]["load_realA"],
						cymcustomerload[row.DeviceNumber]["load_imagA"],
					)
				elif int(row.Phase) == 2:
					cymcustomerload[row.DeviceNumber]["load_realB"] += load_real
					cymcustomerload[row.DeviceNumber]["load_imagB"] += load_imag
					cymcustomerload[row.DeviceNumber][
						...
						float(row.LoadValue2),
						cymcustomerload[row.DeviceNumber]["load_realB"],
						cymcustomerload[row.DeviceNumber]["load_imagB"],
					)
				elif int(row.Phase) == 3:
					cymcustomerload[row.DeviceNumber]["load_realC"] += load_real
					cymcustomerload[row.DeviceNumber]["load_imagC"] += load_imag
					cymcustomerload[row.DeviceNumber][
						...
						float(row.LoadValue2),
						cymcustomerload[row.DeviceNumber]["load_realC"],
						cymcustomerload[row.DeviceNumber]["load_imagC"],
					)
				elif int(row.Phase) == 7:
					cymcustomerload[row.DeviceNumber]["load_realA"] += load_real / 3.0
					cymcustomerload[row.DeviceNumber]["load_imagA"] += load_imag / 3.0
...
def _readCymeTransformer(feederId, modelDir):
	cymxfmr = {}
	CYMXFMR = {"name": None, "equipment_name": None}
...
def _readEqConductor(feederId, modelDir):
	...
	cymeqconductor_db = _csvToDictList(
		pJoin(modelDir, "cymeCsvDump", "CYMEQCONDUCTOR.csv"), feederId
	)
		...
		for row in cymeqconductor_db:
			...
			if row.EquipmentId not in cymeqconductor.keys():
				...
				cymeqconductor[row.EquipmentId][
					"rating.summer_continuous"
				] = row.FirstRating
				cymeqconductor[row.EquipmentId]["geometric_mean_radius"] = (
					float(row.GMR) * m2ft / 100
				)  # GMR is stored in cm. Must convert to ft.
				cymeqconductor[row.EquipmentId]["resistance"] = (
	...
	if len(oh_line_db) == 0:
		...
		for row in oh_line_db:
			...
			if row.EquipmentId not in cymeqoverheadlineunbalanced.keys():
				...
				cymeqoverheadlineunbalanced[row.EquipmentId]["name"] = _fixName(
					row.EquipmentId
				)
				cymeqoverheadlineunbalanced[row.EquipmentId][
					"z11"
				] = "{:0.6f}{:+0.6}j".format(
					float(row.SelfResistanceA) * 5280 / (m2ft * 1000),
					float(row.SelfReactanceA) * 5280 / (m2ft * 1000),
				)  #  Ohm/km. Must convert to Ohm/mile
				cymeqoverheadlineunbalanced[row.EquipmentId][
					"z22"
				] = "{:0.6f}{:+0.6}j".format(
					float(row.SelfResistanceB) * 5280 / (m2ft * 1000),
					float(row.SelfReactanceB) * 5280 / (m2ft * 1000),
				)
				cymeqoverheadlineunbalanced[row.EquipmentId][
					"z33"
				] = "{:0.6f}{:+0.6}j".format(
					float(row.SelfResistanceC) * 5280 / (m2ft * 1000),
					float(row.SelfReactanceC) * 5280 / (m2ft * 1000),
				)
				cymeqoverheadlineunbalanced[row.EquipmentId][
					"z12"
				] = "{:0.6f}{:+0.6}j".format(
					float(row.MutualResistanceAB) * 5280 / (m2ft * 1000),
					float(row.MutualReactanceAB) * 5280 / (m2ft * 1000),
				)
				cymeqoverheadlineunbalanced[row.EquipmentId][
					"z21"
				] = "{:0.6f}{:+0.6}j".format(
				...
				cymeqoverheadlineunbalanced[row.EquipmentId][
					...
					float(row.MutualResistanceBC) * 5280 / (m2ft * 1000),
					float(row.MutualReactanceBC) * 5280 / (m2ft * 1000),
				)
				cymeqoverheadlineunbalanced[row.EquipmentId][
					"z32"
				] = "{:0.6f}{:+0.6}j".format(
				...
				cymeqoverheadlineunbalanced[row.EquipmentId][
					...
					float(row.MutualResistanceCA) * 5280 / (m2ft * 1000),
					float(row.MutualReactanceCA) * 5280 / (m2ft * 1000),
				)
				cymeqoverheadlineunbalanced[row.EquipmentId][
					"z31"
				] = "{:0.6f}{:+0.6}j".format(
...
def _readEqGeometricalArrangement(feederId, modelDir):
	...
	cymeqgeometricalarrangement_db = _csvToDictList(
	...
	if len(cymeqgeometricalarrangement_db) == 0:
		...
		for row in cymeqgeometricalarrangement_db:
			...
			if row.EquipmentId not in cymeqgeometricalarrangement.keys():
				...
				cymeqgeometricalarrangement[row.EquipmentId]["distance_AB"] = (
					math.sqrt(
						(
							float(row.ConductorA_Horizontal)
							- float(row.ConductorB_Horizontal)
						)
						** 2
						+ (
							float(row.ConductorA_Vertical)
							- float(row.ConductorB_Vertical)
						)
						** 2
					)
					* m2ft
				)  # information is stored in meters. must convert to feet.
				cymeqgeometricalarrangement[row.EquipmentId]["distance_AC"] = (
					math.sqrt(
						(
							float(row.ConductorA_Horizontal)
							- float(row.ConductorC_Horizontal)
						)
						** 2
						+ (
							float(row.ConductorA_Vertical)
							- float(row.ConductorC_Vertical)
						)
						** 2
					)
					* m2ft
				)  # information is stored in meters. must convert to feet.
				cymeqgeometricalarrangement[row.EquipmentId]["distance_AN"] = (
					math.sqrt(
						(
							float(row.ConductorA_Horizontal)
							- float(row.NeutralConductor_Horizontal)
						)
						** 2
						+ (
							float(row.ConductorA_Vertical)
							- float(row.NeutralConductor_Vertical)
						)
						** 2
					)
					* m2ft
				)  # information is stored in meters. must convert to feet.
				cymeqgeometricalarrangement[row.EquipmentId]["distance_BC"] = (
					math.sqrt(
						(
							float(row.ConductorC_Horizontal)
							- float(row.ConductorB_Horizontal)
				...
				cymeqgeometricalarrangement[row.EquipmentId]["distance_BN"] = (
					math.sqrt(
						(
							float(row.NeutralConductor_Horizontal)
							- float(row.ConductorB_Horizontal)
				...
				cymeqgeometricalarrangement[row.EquipmentId]["distance_CN"] = (
					math.sqrt(
						(
							float(row.ConductorC_Horizontal)
							- float(row.NeutralConductor_Horizontal)
...
def _readUgConfiguration(feederId, modelDir):
	...
	if len(undergroundcable) == 0:
		...
		for row in undergroundcable:
			...
			if row.EquipmentId not in cymcsvundergroundcable.keys():
				cymcsvundergroundcable[row.EquipmentId] = copy.deepcopy(
					CYMCSVUNDERGROUNDCABLE
				)
				cymcsvundergroundcable[row.EquipmentId]["name"] = _fixName(
					row.EquipmentId
				)
				cymcsvundergroundcable[row.EquipmentId][
					"rating.summer_continuous"
				] = row.FirstRating
				...
				cymcsvundergroundcable[row.EquipmentId][
					...
					row.PositiveSequenceResistance
				)  # leaving as is since z matrix overwrites
				if (
					row.ArmorOuterDiameter is not None
					and row.ArmorOuterDiameter != ""
					and float(row.ArmorOuterDiameter) != 0.0
				):  # jfk
					cymcsvundergroundcable[row.EquipmentId][
						"conductor_diameter"
					] = row.ArmorOuterDiameter
					cymcsvundergroundcable[row.EquipmentId]["conductor_gmr"] = (
						float(row.ArmorOuterDiameter) / 3
					)

					# conversion from cyme's ZeroSequenceResistance/Reactance -> Gridlabd's self/mutual impedances
				z00 = complex(
					float(row.ZeroSequenceResistance), float(row.ZeroSequenceReactance)
				)
				z11 = complex(
				...
				cymcsvundergroundcable[row.EquipmentId]["z11"] = (
					"{:0.6f}".format(Zabc[0][0].real)
					+ "{:+0.6f}".format(Zabc[0][0].imag)
					+ "j"
				)
				cymcsvundergroundcable[row.EquipmentId]["z12"] = (
					"{:0.6f}".format(Zabc[0][1].real)
					+ "{:+0.6f}".format(Zabc[0][1].imag)
					+ "j"
				)
				cymcsvundergroundcable[row.EquipmentId]["z13"] = (
					"{:0.6f}".format(Zabc[0][2].real)
					+ "{:+0.6f}".format(Zabc[0][2].imag)
					+ "j"
				)
				cymcsvundergroundcable[row.EquipmentId]["z21"] = (
					"{:0.6f}".format(Zabc[1][0].real)
					+ "{:+0.6f}".format(Zabc[1][0].imag)
					+ "j"
				)
				cymcsvundergroundcable[row.EquipmentId]["z22"] = (
					"{:0.6f}".format(Zabc[1][1].real)
					+ "{:+0.6f}".format(Zabc[1][1].imag)
					+ "j"
				)
				cymcsvundergroundcable[row.EquipmentId]["z23"] = (
					"{:0.6f}".format(Zabc[1][2].real)
					+ "{:+0.6f}".format(Zabc[1][2].imag)
					+ "j"
				)
				cymcsvundergroundcable[row.EquipmentId]["z31"] = (
					"{:0.6f}".format(Zabc[2][0].real)
					+ "{:+0.6f}".format(Zabc[2][0].imag)
					+ "j"
				)
				cymcsvundergroundcable[row.EquipmentId]["z32"] = (
					"{:0.6f}".format(Zabc[2][1].real)
					+ "{:+0.6f}".format(Zabc[2][1].imag)
					+ "j"
				)
				cymcsvundergroundcable[row.EquipmentId]["z33"] = (
					"{:0.6f}".format(Zabc[2][2].real)
					+ "{:+0.6f}".format(Zabc[2][2].imag)
					+ "j"
				)
	...
	for row in undergroundcableconductor:
		...
		if row.EquipmentId in cymcsvundergroundcable.keys():
			...
			cymcsvundergroundcable[row.EquipmentId]["neutral_gmr"] = (
				float(row.Diameter) / 3
			)
...
def _readEqRegulator(feederId, modelDir):
	...
	cymeqregulator_db = _csvToDictList(
...
def _readEqThreeWAutoXfmr(feederId, modelDir):
	...
	cymeqthreewautoxfmr_db = _csvToDictList(
	...
	if len(cymeqthreewautoxfmr_db) == 0:
		...
		for row in cymeqthreewautoxfmr_db:
			...
			if row.EquipmentId not in cymeqthreewautoxfmr.keys():
				...
				cymeqthreewautoxfmr[row.EquipmentId]["PrimaryRatedCapacity"] = float(
					row.PrimaryRatedCapacity
				)
				cymeqthreewautoxfmr[row.EquipmentId]["PrimaryVoltage"] = (
					float(row.PrimaryVoltage) * 1000.0 / math.sqrt(3.0)
				)
				cymeqthreewautoxfmr[row.EquipmentId]["SecondaryVoltage"] = (
...
def _readEqAutoXfmr(feederId, modelDir):
	...
	cymeqautoxfmr_db = _csvToDictList(
		pJoin(modelDir, "cymeCsvDump", "CYMEQAUTOTRANSFORMER.csv"), feederId
	)
	if len(cymeqautoxfmr_db) == 0:
		warnings.warn(
			"No average autotransformer equipment information was found in CYMEQAUTOTRANSFORMER for feeder id: {:s}.".format(
				feederId
			),
			RuntimeWarning,
		)
		...
		for row in cymeqautoxfmr_db:
			...
			if row.EquipmentId not in cymeqautoxfmr.keys():
				...
				cymeqautoxfmr[row.EquipmentId]["PrimaryRatedCapacity"] = float(
					row.NominalRatingKVA
				)
				cymeqautoxfmr[row.EquipmentId]["PrimaryVoltage"] = (
					float(row.PrimaryVoltageKVLL) * 1000.0 / math.sqrt(3.0)
				)
				cymeqautoxfmr[row.EquipmentId]["SecondaryVoltage"] = (
				...
				cymeqautoxfmr[row.EquipmentId]["impedance"] = "{:0.6f}{:+0.6f}j".format(
					r, x
				)
...
def _readEqXfmr(feederId, modelDir):
	...
	# cymeqautoxfmr_db = equipmentDatabase.execute("SELECT EquipmentId, NominalRatingKVA, PrimaryVoltageKVLL, SecondaryVoltageKVLL, PosSeqImpedancePercent, XRRatio FROM CYMEQAUTOTRANSFORMER").fetchall()
	cymeqxfmr_db = _csvToDictList(
		pJoin(modelDir, "cymeCsvDump", "CYMEQTRANSFORMER.csv"), feederId
	)
	if len(cymeqxfmr_db) == 0:
		warnings.warn(
			"No average transformer equipment information was found in CYMEQTRANSFORMER for feeder id: {:s}.".format(
				feederId
			),
			RuntimeWarning,
		)
		...
		for row in cymeqxfmr_db:
			...
			if row.EquipmentId not in cymeqxfmr.keys():
				...
				cymeqxfmr[row.EquipmentId]["PrimaryRatedCapacity"] = float(
					row.NominalRatingKVA
				)
				cymeqxfmr[row.EquipmentId]["PrimaryVoltage"] = (
					float(row.PrimaryVoltageKVLL) * 1000.0 / math.sqrt(3.0)
				)
				cymeqxfmr[row.EquipmentId]["SecondaryVoltage"] = (
					float(row.SecondaryVoltageKVLL) * 1000.0 / math.sqrt(3.0)
				)
				if (
					cymeqxfmr[row.EquipmentId]["PrimaryVoltage"]
				...
				if r == 0.0:
					...
					x = r * float(row.XRRatio)
				cymeqxfmr[row.EquipmentId]["impedance"] = "{:0.6f}{:+0.6f}j".format(
					r, x
				)
	return cymeqxfmr
...
def _readCymePhotovoltaic(feederId, modelDir):
	cymePhotovoltaic = {}
	CYMEPHOTOVOLTAIC = {"name": None, "configuration": None}
	...
	if len(cymeqbattery_db) == 0:
		...
		for row in cymeqbattery_db:
			...
			if row.DeviceNumber not in cymEqBattery.keys():
				...
				cymEqBattery[row.DeviceNumber]["rated_storage_energy"] = float(
					row.RatedStorageEnergy
				)
				cymEqBattery[row.DeviceNumber][
					"max_charging_power"
				] = row.MaxChargingPower
				cymEqBattery[row.DeviceNumber][
					"max_discharging_power"
				] = row.MaxDischargingPower
				cymEqBattery[row.DeviceNumber]["round_trip_efficiency"] = (
...
def _readCymeBattery(feederId, modelDir):
	cymeBattery = {}
	CYMEBATTERY = {"name": None, "configuration": None, "phase": None}
...
def _readCymeGenerator(feederId, modelDir):
	cymeGenerator = {}
	CYMEGENERATOR = {"name": None, "generation": None, "power_factor": None}
...
def _find_SPCT_rating(load_str):
	...
	)  # jfk.  needs to be in kVA for transformer rating estimation
	spct_rating = [
		5,
		10,
		15,
...
def convertCymeModel(network_db, modelDir, test=False, _type=1, feeder_id=None):
	...
	# -1-CYME CYMSOURCE ***
	cymsource, feeder_id, swingBus = _readCymeSource(feeder_id, _type, modelDir)
	...
	for device, value in cymsectiondevice.keys():
		dType = value["device_type"]
		sName = value["section_name"]

		if dType == 1:
			undergroundline_sections[sName] = key
		elif dType == 3 or dType == 2:
			overheadline_sections[sName] = key
		elif dType == 4:
			regulator_sections[sName] = key
		elif dType == 10:
			recloser_sections[sName] = key
		elif dType == 12:
			sectionalizer_sections[sName] = key
		elif dType == 13:
			if sName not in switch_sections.keys():
				switch_sections[sName] = [
					key
				]  # jfk. could use default dict to make this cleaner
			else:
				switch_sections[sName].append(key)
				# switch_sections[value['section_name']] = key #sometimes have two switches on a section!  this overwrite the first!
		elif dType == 14:
			fuse_sections[sName] = key
		elif dType == 16:
			# sx_section.append(value['section_name']) #this was not needed for PECO files
			reactor_sections[sName] = key
		elif dType == 17:
			capacitor_sections[sName] = key
		elif dType == 20:
			load_sections[sName] = key
		elif dType == 37:
			syncgen_sections[sName] = key
		elif dType == 45:
			if sName not in pv_sections.keys():
				pv_sections[sName] = [key]  # jfk
			else:
				pv_sections[sName].append(key)
		elif (
			dType == 47 or dType == 5
		):  # jfk.  added check for 5, another transformer device
			transformer_sections[sName] = key
		elif dType == 48:
			if dbflag == 0:
				threewautoxfmr_sections[sName] = key
			elif dbflag == 1:
				threewxfmr_sections[sName] = key
		elif dType == 80:
			battery_sections[sName] = key
	...
	for link in cymsection.keys():
		if "from" in cymsection[link].keys() and "to" in cymsection[link].keys():
			if [cymsection[link]["from"], cymsection[link]["to"]] in fromTo or [
				cymsection[link]["to"],
				cymsection[link]["from"],
			] in fromTo:
	...
	for key, value in cymsectiondevice.iteritems():
		...
		if "parent" not in value.keys():
			cymsectiondevice[key]["from"] = cymsection[sName]["from"]
			cymsectiondevice[key]["to"] = cymsection[sName]["to"]
			cymsectiondevice[key]["phases"] = cymsection[sName]["phases"]
			cymsectiondevice[key]["fromLatitude"] = cymsection[sName]["fromX"]
			cymsectiondevice[key]["fromLongitude"] = cymsection[sName]["fromY"]
			cymsectiondevice[key]["toLatitude"] = cymsection[sName]["toX"]
			cymsectiondevice[key]["toLongitude"] = cymsection[sName]["toY"]
			# Create all the node dictionaries
			if value["from"] not in nodes.keys() and value["from"] != swingBus:
				nodes[value["from"]] = {
					"object": "node",
					"name": value["from"],
					"phases": value["phases"],
					"nominal_voltage": str(feeder_VLN),
					"latitude": value["fromLatitude"],
					"longitude": value["fromLongitude"],
				}
			if value["to"] not in nodes.keys() and value["to"] != swingBus:
				nodes[value["to"]] = {
					"object": "node",
					"name": value["to"],
					"phases": value["phases"],
					"nominal_voltage": str(feeder_VLN),
					"latitude": value["toLatitude"],
					"longitude": value["toLongitude"],
				}
		else:
			cymsectiondevice[key]["fromLatitude"] = cymsection[sName]["fromX"]
			cymsectiondevice[key]["fromLongitude"] = cymsection[sName]["fromY"]
	...
	for olc in OH_conductors:
		if olc in cymeqconductor and olc not in ohl_conds:
			ohl_conds[olc] = {
		...
		if ols in cymeqgeometricalarrangement.keys():
			if ols not in ohl_spcs.keys():
				ohl_spcs[ols] = {
					"object": "line_spacing",
					"name": ols,
					"distance_AB": "{:0.6f}".format(
						cymeqgeometricalarrangement[ols]["distance_AB"]
					),
					"distance_AC": "{:0.6f}".format(
						cymeqgeometricalarrangement[ols]["distance_AC"]
					),
					"distance_AN": "{:0.6f}".format(
						cymeqgeometricalarrangement[ols]["distance_AN"]
					),
					"distance_BC": "{:0.6f}".format(
						cymeqgeometricalarrangement[ols]["distance_BC"]
					),
					"distance_BN": "{:0.6f}".format(
						cymeqgeometricalarrangement[ols]["distance_BN"]
					),
					"distance_CN": "{:0.6f}".format(
						cymeqgeometricalarrangement[ols]["distance_CN"]
					),
				}
		else:
			...
			if ohl not in cymoverheadbyphase.keys():
				...
				if ohl not in parallelLinks:
					...
					ohls[ohl + "par1"] = {
						...
						"to": ohl + "parNode",
						"length": "{:0.6f}".format(
							cymoverheadbyphase[ohl]["length"] / 2.0
						),
						"configuration": cymoverheadbyphase[ohl]["configuration"],
					}
					ohls[ohl + "par2"] = {
					...
					nodes[ohl + "parNode"] = {
						...
						"latitude": str(
							(
								float(cymsectiondevice[ohl]["fromLatitude"])
								+ float(cymsectiondevice[ohl]["toLatitude"])
							)
							/ 2.0
						),
						"longitude": str(
							(
								float(cymsectiondevice[ohl]["fromLongitude"])
								+ float(cymsectiondevice[ohl]["toLongitude"])
							)
							/ 2.0
						),
					}
			...
			if ohl not in cymUnbalancedOverheadLine.keys():
				...
				if ohl not in parallelLinks:
					ohls[ohl] = {
						...
						"to": cymsectiondevice[ohl]["to"],
						"length": "{:0.6f}".format(
							cymUnbalancedOverheadLine[ohl]["length"]
						),
						"configuration": cymUnbalancedOverheadLine[ohl][
							"configuration"
						],
					}
				else:
					ohls[ohl + "par1"] = {
						...
						"to": ohl + "parNode",
						"length": "{:0.6f}".format(
							cymUnbalancedOverheadLine[ohl]["length"] / 2.0
						),
						"configuration": cymUnbalancedOverheadLine[ohl][
							"configuration"
						],
		...
		if ulc in cymcsvundergroundcable.keys():
			if ulc + "cond" not in ugl_conds.keys():
				ugl_conds[ulc + "cond"] = {
					...
					"name": ulc + "cond",
					"conductor_resistance": cymcsvundergroundcable[ulc][
						"conductor_resistance"
					],
					...
					"neutral_resistance": cymcsvundergroundcable[ulc][
						"neutral_resistance"
					],
					"neutral_diameter": cymcsvundergroundcable[ulc]["neutral_diameter"],
					"conductor_diameter": cymcsvundergroundcable[ulc][
						"conductor_diameter"
					],
				...
				if config_name not in ugl_cfgs.keys():
					...
					for phase in ph:
						if phase != "D" or phase != "N":
							ugl_cfgs[config_name]["conductor_{:s}".format(phase)] = (
								cymundergroundline[ugl]["cable_id"] + "cond"
							)
				if ugl not in ugls.keys():
					if ugl not in parallelLinks:
						...
						ugls[ugl + "par1"] = {
							...
							"to": ugl + "parNode",
							"length": "{:0.6f}".format(
								cymundergroundline[ugl]["length"] / 2.0
							),
							"configuration": config_name,
						}
						ugls[ugl + "par2"] = {
						...
						nodes[ugl + "parNode"] = {
							...
							"latitude": str(
								(
									float(cymsectiondevice[ugl]["fromX"])
									+ float(cymsectiondevice[ugl]["toX"])
								)
								/ 2.0
							),
							"longitude": str(
								(
									float(cymsectiondevice[ugl]["fromY"])
									+ float(cymsectiondevice[ugl]["toY"])
								)
								/ 2.0
							),
						}
		...
		if cymsectiondevice[rcl]["device_type"] == 10:
			if rcl not in cymrecloser.keys():
				...
				if cymrecloser[rcl]["status"] == 0:
					status = (
						"OPEN"
					)  # was 'CLOSED'  #jfk.  Mistake seemed intentional. Maybe it was just a typo?
				else:
					status = "CLOSED"
		...
		if cymsectiondevice[reactor]["device_type"] == 16:
			if reactor not in cymreactor.keys():
				...
				for ph in reactors[reactor]["phases"]:
					reactors[reactor]["phase_" + ph + "_reactance"] = "{:0.6f}".format(
						Zohms
					)
		...
		if cymsectiondevice[cap]["device_type"] == 17:
			if cap not in cymshuntcapacitor.keys():
				...
				caps[cap] = {
					...
					"dwell_time": "3",
					"latitude": str(
						float(nodes[cymsectiondevice[cap]["parent"]]["latitude"])
						+ random.uniform(-5, 5)
					),
					"longitude": str(
						float(nodes[cymsectiondevice[cap]["parent"]]["longitude"])
						+ random.uniform(-5, 5)
					),
				}
				...
				if caps[cap]["control"] == "VOLT":
					caps[cap]["remote_sense"] = "n" + str(
						cymshuntcapacitor[cap]["remote_sense"]
					)  # jfk.  hacky.  have to add 'n' because it's added later to nodes.
					caps[cap]["voltage_set_high"] = str(
						cymshuntcapacitor[cap]["voltage_set_high"]
						* (1 / 120.0)
						* float(feeder_VLN)
					)
					caps[cap]["voltage_set_low"] = str(
						cymshuntcapacitor[cap]["voltage_set_low"]
						* (1 / 120.0)
						* float(feeder_VLN)
					)
	...
	for load in cymsectiondevice.keys():
		if cymsectiondevice[load]["device_type"] == 20:
			if load not in cymcustomerload.keys():
				...
				load not in loads.keys()
				and cymcustomerload[load]["load_class"] == "commercial"
			):
				loads[load] = {
					"object": "load",
					"name": load,
					...
					for phase in cymcustomerload[load]["phases"]:  # jfk
						if phase not in ["N", "D"]:
							try:
								...
								spctRating = _find_SPCT_rating(
									str(cymcustomerload[load]["constant_power_A"])
								)
								cymcustomerload[load][
									"constant_power_{:s}".format(phase)
								] = cymcustomerload[load]["constant_power_A"]
							spct_cfgs["SPCTconfig{:s}{:s}".format(load, phase)] = {
							...
							tpms["tpm{:s}{:s}".format(load, phase)] = {
								...
								"latitude": str(
									float(
										nodes[cymsectiondevice[load]["parent"]][
											"latitude"
										]
									)
									+ random.uniform(-5, 5)
	...
	for reg in cymsectiondevice.keys():
		if cymsectiondevice[reg]["device_type"] == 4:
			if reg not in cymregulator.keys():
				...
				reg_nominalvoltage = (
					float(cymeqregulator[regEq]["nominal_voltage"]) * 1000.0
				)
		...
		if cymsectiondevice[pv]["device_type"] == 45:
			if pv not in cymphotovoltaic.keys():
				...
				pv_sec[pv] = {
					"object": "solar",
					"name": pv,
					"efficiency": cymeqphotovoltaic[
						cymphotovoltaic[pv]["configuration"]
					]["efficiency"],
					"area": 1000
					* 0.075
					* float(
						cymeqphotovoltaic[cymphotovoltaic[pv]["configuration"]][
							"voltage"
						]
					)
					* float(
	...
	for bat in cymsectiondevice.keys():
		if cymsectiondevice[bat]["device_type"] == 80:
			if bat not in cymbattery.keys():
				...
				bat_sec[cymsectiondevice[bat]["section_name"]] = {
					"object": "meter",
				...
				bat_sec[bat + "inv"] = {
					...
					"rated_power": cymeqbattery[cymbattery[bat]["configuration"]][
						"rated_storage_energy"
					],
					"charge_off_threshold": 6148,
					"max_charge_rate": cymeqbattery[cymbattery[bat]["configuration"]][
						"max_charging_power"
					],
					"max_discharge_rate": cymeqbattery[
						cymbattery[bat]["configuration"]
					]["max_discharging_power"],
					"discharge_lockout_time": 60,
					"charge_lockout_time": 60,
					"inverter_efficiency": cymeqbattery[
						cymbattery[bat]["configuration"]
				...
				bat_sec[bat] = {
					...
					"round_trip_efficiency": cymeqbattery[
						cymbattery[bat]["configuration"]
					]["round_trip_efficiency"],
					"generator_mode": "SUPPLY_DRIVEN",
					"generator_status": "ONLINE",
					"battery_state": 1.0,
					"battery_capacity": cymeqbattery[cymbattery[bat]["configuration"]][
						"rated_storage_energy"
					],
					"battery_type": "LI_ION",
				...
				if xfmrEq not in cymeqautoxfmr.keys():
					...
					if xfmrEq not in xfmr_cfgs.keys():
						xfmr_cfgs[xfmrEq] = {
							...
							"name": xfmrEq + suffix,
							"connect_type": "WYE_WYE",
							"primary_voltage": "{:0.6f}".format(
								cymeqautoxfmr[xfmrEq]["PrimaryVoltage"] * math.sqrt(3)
							),  # jfk
							"secondary_voltage": "{:0.6f}".format(
								cymeqautoxfmr[xfmrEq]["SecondaryVoltage"] * math.sqrt(3)
							),  # jfk
							"impedance": cymeqautoxfmr[xfmrEq]["impedance"],
							"power_rating": "{:0.0f}".format(
								cymeqautoxfmr[xfmrEq]["PrimaryRatedCapacity"]
							),
						}
						for phase in ph:
							if phase not in ["N", "D"]:
				...
				if xfmrEq not in cymeqxfmr.keys():
					...
					if xfmrEq not in xfmr_cfgs.keys():
						xfmr_cfgs[xfmrEq] = {
							...
							"connect_type": "WYE_WYE",
							"primary_voltage": "{:0.6f}".format(
								cymeqxfmr[xfmrEq]["PrimaryVoltage"] * math.sqrt(3)
							),  # jfk
							"secondary_voltage": "{:0.6f}".format(
								cymeqxfmr[xfmrEq]["SecondaryVoltage"] * math.sqrt(3)
							),  # jfk
							# 'resistance': '0.000000001',
							# 'reactance': '0.0000000006',
							"impedance": cymeqxfmr[xfmrEq]["impedance"],
							"power_rating": "{:0.0f}".format(
								cymeqxfmr[xfmrEq]["PrimaryRatedCapacity"]
							),
						}
						for phase in ph:
							if phase not in ["N", "D"]:
								xfmr_cfgs[xfmrEq][
									...
									cymeqxfmr[xfmrEq]["PrimaryRatedCapacity"] / phNum
								)
				if xfmr not in xfmrs.keys():
					xfmrs[xfmr] = {
						"object": "transformer",
				...
				if xfmrEq not in cymeq3wautoxfmr.keys():
					...
					if xfmrEq not in xfmr_cfgs.keys():
						xfmr_cfgs[xfmrEq] = {
							...
							"connect_type": "WYE_WYE",
							"primary_voltage": "{:0.6f}".format(
								cymeq3wautoxfmr[xfmrEq]["PrimaryVoltage"]
							),
							"secondary_voltage": "{:0.6f}".format(
								cymeq3wautoxfmr[xfmrEq]["SecondaryVoltage"]
							),
							"impedance": cymeq3wautoxfmr[xfmrEq]["impedance"],
							"power_rating": "{:0.0f}".format(
								cymeq3wautoxfmr[xfmrEq]["PrimaryRatedCapacity"]
							),
						}
						for phase in ph:
							if phase not in ["N", "D"]:
		...
		if (
			...
			if swToNodes[glmTree[x]["to"]] > 1:
				...
				for y in glmTree.keys():
					if (
						glmTree[y].get("name", "") == glmTree[x]["to"]
					):  # jfk was glmTree[y].get['name', '']  #typo with the brackets
						deleteKeys.append(y)
