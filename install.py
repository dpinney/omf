...
if major_platform == "Linux" and "ubuntu" in linux_distro:
	...
	os.system(f'{sys.executable} -m pip install pygraphviz --global-option=build_ext --global-option="-I/opt/homebrew/include/" --global-option="-L/opt/homebrew/lib/graphviz" ')
	pipInstallInOrder(f"{sys.executable} -m pip")
	os.system(f"{sys.executable} setup.py develop")
else:
