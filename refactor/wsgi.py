...
@app.route('/')
def root():
	analyses = datastore.listAll('StudyGridlab')
	metadatas = [datastore.getMetadata(x) for x in analyses]
	return flask.render_template('home.html', metadatas=metadatas)

@app.route('/newAnalysis/')
def newAnalysis():
	# Get all the template TMY2s available.
	tmy2s = datastore.listAll('StudyGridlab')
	# Get all the template feeders available.
	feeders = datastore.listAll('Grid')
